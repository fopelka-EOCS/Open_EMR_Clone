...
*/
    ...
    /**
     * Test logHttpRequest skips logging when skipAuditLog flag is set
     */
    public function testLogHttpRequestSkipsWhenSkipAuditLogFlagSet(): void
    {
        // Enable audit logging and HTTP request logging
        $GLOBALS['enable_auditlog'] = true;
        $GLOBALS['audit_events_http-request'] = true;
        // Set skipAuditLog flag (used by health checks)
        $GLOBALS['skipAuditLog'] = true;
        ...
        try {
            ...
            // Mock newEvent method to ensure it's not called when skipAuditLog is set
            $loggerMock = $this->getMockBuilder(EventAuditLogger::class)
                ->onlyMethods(['newEvent'])
                ->disableOriginalConstructor()
                ->getMock();

            // newEvent should never be called when skipAuditLog is set
            $loggerMock->expects($this->never())->method('newEvent');
    ...
    /**
     * Test logHttpRequest still logs when skipAuditLog flag is not set
     */
    public function testLogHttpRequestLogsNormallyWhenSkipAuditLogNotSet(): void
    {
        // Enable audit logging and HTTP request logging
        $GLOBALS['enable_auditlog'] = true;
        $GLOBALS['audit_events_http-request'] = true;
        // Ensure skipAuditLog is not set
        unset($GLOBALS['skipAuditLog']);

        // Set up server variables for a normal endpoint
        try {
            $_SERVER['REQUEST_METHOD'] = 'GET';
            $_SERVER['SCRIPT_NAME'] = '/interface/patient_file/summary/demographics.php';
            $_SESSION['authUser'] = 'test_user';
            ...
            // Mock newEvent method to verify it IS called when skipAuditLog is not set
            $loggerMock = $this->getMockBuilder(EventAuditLogger::class)
                ->onlyMethods(['newEvent'])
                ->disableOriginalConstructor()
                ->getMock();

            // newEvent SHOULD be called once for normal endpoints
            $loggerMock->expects($this->once())
                ->method('newEvent')
                ->with(
                    'http-request-select',
            ...
            // Call logHttpRequest - should log normally
            $loggerMock->logHttpRequest();
        } finally {
            // Restore original server variables
            if (isset($this->originalServer['REQUEST_METHOD'])) {
    ...
    /**
     * Test auditSQLEvent skips logging when skipAuditLog flag is set
     */
    public function testAuditSQLEventSkipsWhenSkipAuditLogFlagSet(): void
    {
        ...
        try {
            // Mock recordLogItem to ensure it's not called when skipAuditLog is set
            $loggerMock = $this->getMockBuilder(EventAuditLogger::class)
                ->onlyMethods(['recordLogItem'])
                ->disableOriginalConstructor()
                ->getMock();

            // recordLogItem should never be called when skipAuditLog is set
